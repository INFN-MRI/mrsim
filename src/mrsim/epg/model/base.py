...
class BaseSimulator:
    ...
    # main properties
    T1: float | npt.NDArray[float] | torch.FloatTensor  # ms
    T2: float | npt.NDArray[float] | torch.FloatTensor  # ms
    diff: str | tuple[str] = None
    device: str = "cpu"
    B1: float | npt.NDArray[float] | torch.FloatTensor = None
    B0: float | npt.NDArray[float] | torch.FloatTensor = None  # Hz
    ...
    T2star: float | npt.NDArray[float] | torch.FloatTensor = None  # ms
    D: float | npt.NDArray[float] | torch.FloatTensor = None  # um**2 / ms
    v: float | npt.NDArray[float] | torch.FloatTensor = None  # cm / s
    moving: bool = False
    chemshift: float | npt.NDArray[float] | torch.FloatTensor = None  # Hz

    # bloch-mcconnell parameters
    T1bm: float | npt.NDArray[float] | torch.FloatTensor = None  # ms
    T2bm: float | npt.NDArray[float] | torch.FloatTensor = None  # ms
    kbm: float | npt.NDArray[float] | torch.FloatTensor = None  # Hz
    weight_bm: float | npt.NDArray[float] | torch.FloatTensor = None
    chemshift_bm: float | npt.NDArray[float] | torch.FloatTensor = None  # Hz

    # bloch-mt parameters
    kmt: float | npt.NDArray[float] | torch.FloatTensor = None  # Hz
    weight_mt: float | npt.NDArray[float] | torch.FloatTensor = None

    # fields
    B1Tx2: float | npt.NDArray[float] | torch.FloatTensor = None
    B1phase: float | npt.NDArray[float] | torch.FloatTensor = None  # deg
