...
from deepmr.bloch import base, ops
...
def ssfp_sim(flip, TR, T1, T2, diff=None, device="cpu", TI=None, **kwargs):
    ...
    Kwargs (sequence):
        ...
        rf_envelope (array_like): rf time envelope when flip = 1 [deg].
            Together with "duration", it is Uìused to compute slice profile
...
def SSFPFidStep(states, TE, TR, T1, T2, weight, k, chemshift, D, v, grad_props):
    ...
    if len(TE) == 1:
        if TE == 0.0:
            ETE = ops.Identity(name="Relaxation until TE")
            WTE = ops.Identity(name="Magnetization inflow / washout until TE")
            TEop = ops.CompositeOperator(WTE, ETE)
        else:
            if hasK:
                ETE = ops.Relaxation(
                    device, TE, T1, T2, weight, k, chemshift, name="Relaxation until TE"
                )
            else:
                ETE = ops.Relaxation(device, TE, T1, T2, name="Relaxation until TE")
            if hasV:
                WTE = ops.FlowWash(
                    device,
                    TE,
                    v,
                    name="Magnetization inflow / washout until TE",
                    **grad_props,
                )
            else:
                WTE = ops.Identity(name="Magnetization inflow / washout until TE")
        ...
        for te in TE:
            ...
            if hasV and "moving" in states:
                ...
                WTE = ops.Identity(name="Magnetization inflow / washout until TE")
            TEop.append(ops.CompositeOperator(copy.deepcopy(WTE), copy.deepcopy(ETE)))

    # prep until TR
    if hasK:
        ETETR = ops.Relaxation(
            device,
            TR - TE[-1],
            T1,
            T2,
            ...
            k,
            chemshift,
            name="Relaxation until next TR",
        )
    else:
        ETETR = ops.Relaxation(
            device, TR - TE[-1], T1, T2, name="Relaxation until next TR"
        )
    ...
    if hasV and "moving" in states:
        ...
        WTETR = ops.Identity(name="Magnetization inflow / washout until next TR")

    # set up diffusion
    if hasD:
        D = ops.DiffusionDamping(
            device, tau, D, nstates, name="Diffusion until next TR", **grad_props
        )
    else:
    ...
    if hasV:
        J = ops.FlowDephasing(
            device, tau, v, nstates, name="Flow dephasing until next TR", **grad_props
        )
    else:
    ...
    TETRop = ops.CompositeOperator(S, WTETR, J, D, ETETR, name="SSFP TE-TR Propagator")
